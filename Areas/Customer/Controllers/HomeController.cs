...
namespace CultureWeb.Areas.Customer.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public ViewResult List(string search)
        {
            ...
            var model = from m in _db.Products.Include(m => m.SubCategories).Include(m => m.Reviews).Include(m => m.ProductPrices)
                        select m;
        ...
        public async Task<IActionResult> Index(string id, string sortOrder, string currentFilter, string searchString, int? pageNumber)
        {
            ...
            var products = _db.Products.Include(c => c.SubCategories).Include(c => c.Reviews).Include(c => c.ProductPrices);
            int pageSize = 30;
        ...
        public IActionResult Cart()
        {
            ...
            foreach (var kvp in productQuantities)
            {
                ...
                // Use Entity Framework's Include method to include ProductPrices
                var product = _db.Products
                    .Include(p => p.ProductPrices)
